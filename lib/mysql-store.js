...
const ProtoStore = require('passwordless-tokenstore');
const mysql = require('mysql');

/**
 * Constructor of Store
 * @param {Object} config Connection config (see https://www.npmjs.com/package/mysql#pooling-connections)
...
function Store(config, options) {
  ...
  this._client = mysql.createPool(config);
  this._client.query(
...
Store.prototype.authenticate = function(token, uid, callback) {
  ...
  self._client.query(
    'SELECT * FROM ' + self._table + ' WHERE uid=?',
    [uid],
    function(err, result) {
      if (err) {
 ...
 */
Store.prototype.storeOrUpdate = function(
  token,
  uid,
  msToLive,
  originUrl,
  ...
  bcrypt.hash(token, 10, function(err, hashedToken) {
    ...
    self._client.query(
      'INSERT INTO ' +
        self._table +
        '(uid, token, origin, ttl) VALUES(?, ?, ?, ?)',
      [uid, hashedToken, originUrl, Date.now() + msToLive],
      function(err) {
        if (err) {
          self._client.query(
            'UPDATE ' +
              self._table +
              ' SET token=?, origin=?, ttl=? WHERE uid=?',
            [hashedToken, originUrl, Date.now() + msToLive, uid],
            function(err) {
              if (err) {
                callback(err);
              } else {
...
Store.prototype.invalidateUser = function(uid, callback) {
  ...
  this._client.query(
    'DELETE FROM ' + this._table + ' WHERE uid=?',
    [uid],
    function(err) {
      if (err) {
        return callback(err);
...
Store.prototype.length = function(callback) {
  ...
  this._client.query('SELECT COUNT(uid) FROM ' + this._table, function(
    err,
    result
  ) {
    if (err) {
